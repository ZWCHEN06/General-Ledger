                                             ...
                                             const QVariant &categoryId)
{
    ...
    int existingSubcategoryId = -1;
    QString existingSubcategoryName;
    const std::optional<Transaction> existingTransaction = m_transactionRepository->getTransactionById(id);
    if (existingTransaction.has_value()
        && existingTransaction->categoryId().has_value()
        && parsedCategoryId.has_value()
        && existingTransaction->categoryId().value() == parsedCategoryId.value()) {
        existingSubcategoryId = existingTransaction->subcategoryId();
        existingSubcategoryName = existingTransaction->subcategory();
    }
